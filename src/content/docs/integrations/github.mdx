...
| Outcome | Behavior |
|---------|----------|
| Transient provider outage | Defers without a PR comment, keeps the status `pending`, and retries with exponential backoff. The first delay is 2 minutes, it doubles up to a 1 hour cap, and transient retries give up after 72 hours. |
